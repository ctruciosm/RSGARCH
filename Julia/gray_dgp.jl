    ...
    if (!time_varying)
     ...
     if distri == "norm"
         ...
         for i = 2:ntot
             ...
             deno = sqrt(7/5) / sqrt(h[i - 1, 1]) * pdf(TDist(7), r[i - 1] * sqrt(7/5) / sqrt(h[i - 1, 1])) * Pt[i - 1] + sqrt(7/5) / sqrt(h[i - 1, 2]) * pdf(TDist(7), r[i - 1] * sqrt(7/5) / sqrt(h[i - 1, 2])) * (1 - Pt[i - 1]);
             Pt[i] = numA/deno + numB/deno;
             h[i, k + 1] = Pt[i] * h[i, 1] + (1 - Pt[i]) * h[i, 2];
             s[i] = wsample([1, 2], P[:, s[i-1]])[1];
             r[i] = e[i] * sqrt(h[i, s[i]]);
     ...
     P[1, 1] = cdf(Normal(), C[1] + D[1] * 0);              ## 0 é a melhor opção?
     P[2, 2] = cdf(Normal(), C[2] + D[2] * 0);              ## 0 é a melhor opção?
     P[2, 1] = 1.0 - P[1, 1];
     P[1, 2] = 1.0 - P[2, 2];
     p = P[1, 1];
     q = P[2, 2];
